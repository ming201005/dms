...
public interface SysRoleTableDao extends BaseMapper<SysRoleTable> {
    ...
    @Select("SELECT  * from sys_role_table  where role_name<>'ROLE_ADMIN'")
    /**
     * 查询所有的权限
     * @return
     */
    List<SysRoleTable> getRoles();
    ...
    @Select("SELECT \n" +
            ...
            "    t2.role_id, \n" +
            "    CASE WHEN t1.user_type =1 THEN '学生' ELSE  '非学生' END AS pid ,\n"+
            ...
            "    JOIN organization d  ON d.id = t1.organization_id order by d.id")
    /**
